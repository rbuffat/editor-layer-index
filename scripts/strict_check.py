...
def check_wms(source, info_msgs, warning_msgs, error_msgs):
    ...
    if "layers" in wms_args:
        ...
        if len(not_found_layers) > 0:
            error_msgs.append(
                "Layers '{}' not advertised by WMS GetCapabilities "
        ...
        if geom is not None and geom.is_valid:
            ...
            if max_outside > 5.0:
                error_msgs.append(
                    "{}% of geometry is outside of the layers bounding box. "
...
logging.info(f"arguments.path: {arguments.path}")
for filename in arguments.path:
    logging.info(f"file: {filename} exists: {os.path.exists(filename)}")
    ...
    try:
        ...
        if "attribution" in source["properties"]:
            if "url" in source["properties"]["attribution"]:
                ...
                try:
                    ...
                    if not r.status_code == 200:
                        error_msgs.append(
                            "{}: attribution url {} is not reachable: HTTP code: {}".format(
                    ...
                    error_msgs.append(
                        "{}: attribution url {} is not reachable: {}".format(
                            filename, url, str(e)
                        )
                    )
        ...
        if "icon" in source["properties"] and source["properties"]["icon"].startswith(
            ...
            try:
                ...
                if not r.status_code == 200:
                    error_msgs.append(
                        "{}: icon url {} is not reachable: HTTP code: {}".format(
                            filename, url, r.status_code
                        )
                    )
                ...
                error_msgs.append(
                    "{}: icon url {} is not reachable: {}".format(filename, url, str(e))
                )
        ...
        if source["properties"]["type"] == "tms":
            ...
            warning_msgs.append(
                "Imagery type {} is currently not checked.".format(
